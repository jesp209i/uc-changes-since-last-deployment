...
async function patch(patchFileLocation, githubToken, run2) {
  return await new Promise(async (resolve, reject) => {
    try {
      ...
      await (0, import_exec.exec)("git", ["config", "user.name", `"Build pipeline"`]);
      await (0, import_exec.exec)("git", ["config", "user.email", "cicd+github@umbraco.com"]);
      await (0, import_exec.exec)("git", ["switch", "-c", `${branchName}`]);
      await (0, import_exec.exec)("git", ["apply", "-v", `${patchFileLocation}`]);
      (0, import_io.rmRF)(patchFileLocation);
      await (0, import_exec.exec)("git", ["commit", "-a", "-m", `"Auto Updated changes from remote for build ${run2}"`]);
      await (0, import_exec.exec)("git", ["remote", "add", "tmp-pusher", `https://${githubToken}@github.com/jesp209i/pipelines-r-us`]);
      await (0, import_exec.exec)("git", ["push", "--set-upstream", "tmp-pusher", `${branchName}`]);
      return resolve();
    } catch (error) {
      return reject(error);
    }
...
async function run() {
  ...
  const githubToken = (0, import_core2.getInput)("github-pat-token");
  const currentRun = (0, import_core2.getInput)("pipeline-run");
